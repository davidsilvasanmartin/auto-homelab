...
Interesting guides:
    - https://www.seanh.cc/2022/04/03/restic/
...
class ResticToBackblazeBackup:
    ...
    def _run_command(self, command, check=True):
        ...
        try:
            result = subprocess.run(command, env=self.env, check=check, capture_output=True, text=True)
            return result
    ...
    def create_backup(self):
        ...
        result = self._run_command(
            ["restic", "backup", str(self.backup_path), "--tag", f"automatic-{timestamp}", "--verbose"]
        )
    ...
    def prune_old_backups(self):
        ...
        result = self._run_command(
            ["restic", "forget", "--keep-within", f"{self.retention_days}d", "--prune"]
        )
    ...
    def restore_latest(self, target_dir):
        ...
        result = self._run_command(["restic", "restore", "latest", "--target", str(target_path), "--verbose"])
...
def main():
    ...
    parser.add_argument(
        "--restore-dir", help="Directory where to restore files (only used with restore command)"
    )
    parser.add_argument(
        "--snapshot-id", help="Snapshot ID to list files from (required for ls-files command)"
    )
